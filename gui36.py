...
def compute_model():
	...
	p = subprocess.call(["build27.bat", ""])
	from model import model
	number_of_threads = 20;
	from multiprocessing.pool import ThreadPool
	pool = ThreadPool(processes=number_of_threads+2)

	async_result = [pool.apply_async(model) for i in range(number_of_threads)];
	print("computing....");
	return_val = [async_result[i].get() for i in range(number_of_threads)];
...
def plot_figure():
	...
	p = ax.scatter(X_points, Y_points,c =Z_points, s=5, edgecolor='',cmap=matplotlib.cm.jet,picker = 1)
...
E_File.place(bordermode=OUTSIDE, x=160,y=170,height=30, width=600)
...
B3.place(bordermode=OUTSIDE, x=350,y=100,height=30, width=100)



B4 = Button(text='save...', font=selected_font,command = save_results_file,state = "disabled")
B4.place(bordermode=OUTSIDE, x=20,y=150,height=30, width=100)

B5 = Button(text='load...', font=selected_font,command = load_results_file)
B5.place(bordermode=OUTSIDE, x=20,y=180,height=30, width=100)
...
C1.place(bordermode=OUTSIDE, x=20,y=250,height=30, width=200)
...
C2.place(bordermode=OUTSIDE, x=300,y=250,height=30, width=200)
C2.bind("<<ComboboxSelected>>", combobox_selection)
C3.place(bordermode=OUTSIDE, x=570,y=250,height=30, width=200)
...
B6.place(bordermode=OUTSIDE, x=300,y=300,height=30, width=100)


S_tel = StringVar()
S_tel.set("0.1")
Stel = Spinbox(textvariable = S_tel,from_=0, to=2,increment=0.01)
Stel.place(bordermode=OUTSIDE, x=800,y=250,height=30, width=50)
